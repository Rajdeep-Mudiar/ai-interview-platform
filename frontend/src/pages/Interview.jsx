...
ChartJS.register(
  ...
  const handleInterviewEnd = async (status) => {
    ...
    try {
      if (status === "terminated") {
        await axiosClient.post("/interview/terminate", {
          candidate_id: user.id,
          candidate_name: user.name,
          job_id: jobId,
          job_title: location.state?.jobTitle || "Job Position",
          integrity_score: integrity,
          time_taken_seconds: 300 - time,
        });
      } else {
        // Prepare items for evaluation
        const items = questions.map((q, idx) => ({
          question: typeof q === 'object' ? q.question : q,
          preferred_answer: typeof q === 'object' ? q.preferred_answer : "",
          candidate_answer: answers[idx] || ""
        }));

        await axiosClient.post("/interview/evaluate", {
          candidate_id: user.id,
          candidate_name: user.name,
          job_id: jobId,
          job_title: location.state?.jobTitle || "Job Position",
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              {jobDetails ? `${jobDetails.title} @ ${jobDetails.company || "AI Platform"}` : "AI Technical Interview"}
            </h1>
