...
function RecruiterDashboard() {
  ...
  const addQA = () => {
    setJobForm(prev => ({
      ...prev,
      qa_pairs: [...prev.qa_pairs, { question: "", answer: "" }]
    }));
  };

  const updateQA = (index, field, value) => {
    const newQA = [...jobForm.qa_pairs];
    newQA[index][field] = value;
    setJobForm(prev => ({ ...prev, qa_pairs: newQA }));
  };

  const removeQA = (index) => {
    if (jobForm.qa_pairs.length > 1) {
      const newQA = jobForm.qa_pairs.filter((_, i) => i !== index);
      setJobForm(prev => ({ ...prev, qa_pairs: newQA }));
    }
  };
  ...
  async function createJob(e) {
    ...
    const validPairs = jobForm.qa_pairs.filter(p => p.question.trim());
    const questions = validPairs.map(p => p.question.trim());
    const preferred_answers = validPairs.map(p => p.answer.trim());
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-blue-600 font-bold mt-1 uppercase tracking-wider">
                          Step {formStep} of {totalSteps}: {
                            ...
                            formStep === 2 ? "Candidate Requirements" :
                            formStep === 3 ? "Role & Benefits" : "AI Interview Setup"
                          }
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        {[1, 2, 3, 4].map(i => (
                          <div 
                            key={i} 
                            className={`h-2 w-8 rounded-full transition-all duration-300 ${i <= formStep ? 'bg-blue-600' : 'bg-blue-200'}`}
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                      {formStep === 1 && (
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                          ...
                          <div className="grid sm:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-2">
                              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Work Type</label>
                              <select
                                className="w-full h-14 px-5 rounded-2xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-sm font-bold text-slate-700 appearance-none cursor-pointer"
                                value={jobForm.work_type}
                                onChange={(e) => updateJobForm("work_type", e.target.value)}
                              >
                      ...
                      {formStep === 2 && (
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                          <div className="grid sm:grid-cols-2 gap-6">
                            <div className="space-y-2">
                              ...
                              <input
                                ...
                                onChange={(e) => updateJobForm("qualification", e.target.value)}
                                placeholder="e.g. Bachelor's in CS or equivalent"
                              />
                            </div>
                            <div className="space-y-2">
                              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Work Experience</label>
                              <input
                                className="w-full h-14 px-5 rounded-2xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-sm font-bold text-slate-700"
                                value={jobForm.work_experience}
                                onChange={(e) => updateJobForm("work_experience", e.target.value)}
                          ...
                          <div className="space-y-2">
                            ...
                            <textarea
                              className="w-full min-h-[120px] p-5 rounded-2xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-sm font-bold text-slate-700 resize-none"
                      ...
                      {formStep === 3 && (
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                          <div className="grid sm:grid-cols-2 gap-6">
                            <div className="space-y-2">
                              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Key Responsibilities</label>
                              <textarea
                                className="w-full min-h-[100px] p-5 rounded-2xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-sm font-bold text-slate-700 resize-none"
                                value={jobForm.key_responsibilities}
                                onChange={(e) => updateJobForm("key_responsibilities", e.target.value)}
                                placeholder="What will they be doing daily?"
                              />
                            </div>
                            <div className="space-y-2">
                              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Requirements / Skills</label>
                              <textarea
                                className="w-full min-h-[100px] p-5 rounded-2xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-sm font-bold text-slate-700 resize-none"
                                value={jobForm.requirements}
                                onChange={(e) => updateJobForm("requirements", e.target.value)}
                                placeholder="Technical skills and soft skills..."
                              />
                            </div>
                          </div>
                          <div className="grid sm:grid-cols-2 gap-6">
                            <div className="space-y-2">
                              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Benefits</label>
                              <input
                                className="w-full h-14 px-5 rounded-2xl bg-slate-50 ring-1 ring-slate-200 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-sm font-bold text-slate-700"
                                value={jobForm.benefits}
                                onChange={(e) => updateJobForm("benefits", e.target.value)}
                      ...
                      {formStep === 4 && (
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
                              <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">AI Interview Q&A Pairs</label>
                              <Button 
                                type="button" 
                                onClick={addQA} 
                                variant="secondary" 
                                className="h-8 px-4 text-[10px] font-black rounded-xl border-blue-100 text-blue-600 hover:bg-blue-50"
                              >
                                + Add Question
                              </Button>
                            </div>
                            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                              {jobForm.qa_pairs.map((qa, index) => (
                                <div key={index} className="p-5 rounded-3xl bg-slate-50 ring-1 ring-slate-200 space-y-4 relative group/qa">
                                  <div className="space-y-2">
                                    <input
                                      className="w-full h-10 px-4 rounded-xl bg-white ring-1 ring-slate-100 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-sm font-bold text-slate-700"
                                      value={qa.question}
                                      onChange={(e) => updateQA(index, "question", e.target.value)}
                                      placeholder={`Question ${index + 1}`}
                                    />
                                    <textarea
                                      className="w-full min-h-[60px] p-4 rounded-xl bg-white ring-1 ring-slate-100 focus:ring-2 focus:ring-blue-600 outline-none transition-all text-xs font-medium text-slate-600 resize-none"
                                      value={qa.answer}
                                      onChange={(e) => updateQA(index, "answer", e.target.value)}
                                      placeholder="Ideal answer for AI evaluation..."
                                    />
                                  </div>
                                  {jobForm.qa_pairs.length > 1 && (
                                    <button 
                                      type="button" 
                                      onClick={() => removeQA(index)}
                                      className="absolute -top-2 -right-2 h-8 w-8 rounded-full bg-white shadow-md ring-1 ring-slate-200 flex items-center justify-center text-rose-500 hover:bg-rose-50 opacity-0 group-hover/qa:opacity-100 transition-all"
                                    >
                                      <TrashIcon />
                                    </button>
                                  )}
                  ...
                  jobs.map((job) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1.5 mb-4">
                        {job.company && <div className="text-[10px] font-bold text-slate-500">{job.company}</div>}
                        <div className="flex items-center gap-3">
                          {job.location && <div className="text-[9px] font-medium text-slate-400">{job.location}</div>}
                          {job.salary && <div className="text-[9px] font-medium text-slate-400">{job.salary}</div>}
                        </div>
                      </div>
