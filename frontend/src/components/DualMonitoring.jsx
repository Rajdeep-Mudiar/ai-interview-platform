...
const DualMonitoring = ({
  ...
  const syncSessionState = async (id) => {
    if (!id) {
      return;
    }

    try {
      const sessionRes = await axiosClient.get(`/monitoring/sessions/${id}`);
      setSessionData(sessionRes.data);
      setIsMobileConnected(Boolean(sessionRes.data.mobile_connected));
      if (typeof onMobileStatusChange === "function") {
        onMobileStatusChange(Boolean(sessionRes.data.mobile_connected));
      }
    } catch (err) {
  ...
  const buildMobileMonitoringLink = (id, fallbackLink) => {
    if (!id || typeof window === "undefined") {
      return fallbackLink || "";
    }

    const appendServiceParams = (rawUrl) => {
      try {
        const url = new URL(rawUrl);
        url.searchParams.set("api", API_BASE_URL);
        url.searchParams.set("ai", AI_BASE_URL);
        return url.toString();
      } catch {
        return rawUrl;
      }
    ...
    const configuredBase = (
      import.meta.env.VITE_MOBILE_MONITORING_BASE_URL || ""
    )
      .trim()
      .replace(/\/$/, "");
    if (configuredBase) {
      return appendServiceParams(`${configuredBase}/mobile-monitoring/${id}`);
    }
    ...
    if (fallbackLink) {
      try {
        ...
        const fallbackIsLocal =
          parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1";

        // If backend returned localhost but current page is opened on a reachable host/tunnel,
        // rewrite QR link to that host so mobile can access it.
        if (!isLocalHost && fallbackIsLocal) {
          parsed.protocol = window.location.protocol;
          parsed.hostname = window.location.hostname;
          if (window.location.port) {
            parsed.port = window.location.port;
          }
          return appendServiceParams(parsed.toString());
        }
      } catch {
    ...
    // Prefer current origin when app is already opened on a reachable host/tunnel.
    if (!isLocalHost) {
      return appendServiceParams(
        `${window.location.origin}/mobile-monitoring/${id}`,
      );
    }

    return appendServiceParams(
      fallbackLink || `${window.location.origin}/mobile-monitoring/${id}`,
    );
  };
  ...
  useEffect(() => {
    const initializeSession = async () => {
      try {
        ...
        setMobileLink(
          buildMobileMonitoringLink(
            response.data.session_id,
            response.data.mobile_link,
          ),
        );
      } catch (err) {
    ...
    if (sessionId) {
      const fetchData = async () => {
        try {
          ...
          setAlerts(
            logsRes.data.filter((log) => log.event !== "device_connected"),
          );
        } catch (err) {
  ...
  }, [sessionId, onMobileStatusChange]);

  useEffect(() => {
    if (!sessionId || isMobileConnected) {
      return;
    }
  ...
  }, [sessionId, isMobileConnected, onMobileStatusChange]);

  // WebSocket for real-time alerts
  useEffect(() => {
    if (sessionId) {
      socketRef.current = new WebSocket(
        `${monitoringWsBase}/monitoring/ws/${sessionId}`,
      );
      ...
      socketRef.current.onmessage = (event) => {
        ...
        if (data.type === "suspicious_activity") {
          ...
          if (typeof onSuspiciousActivity === "function") {
            onSuspiciousActivity(data.payload);
          }
          // Refresh session data to get updated integrity score
          syncSessionState(sessionId);

          if (Notification.permission === "granted") {
            new Notification("Suspicious Activity", {
              body: data.payload.message,
            });
          }
        } else if (data.type === "device_status") {
          if (
            data.payload.device === "mobile" &&
            data.payload.status === "connected"
          ) {
            setIsMobileConnected(true);
            if (typeof onMobileStatusChange === "function") {
              onMobileStatusChange(true);
            }
          }
        }
      };
  ...
  }, [sessionId, onSuspiciousActivity, onMobileStatusChange]);
      ...
      <CardHeader className="p-6 pb-4 border-b border-slate-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div
              className={`w-2.5 h-2.5 rounded-full ${isMobileConnected ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]" : "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.6)] animate-pulse"}`}
            ></div>
          ...
          {sessionData && (
            <div className="px-3 py-1 rounded-lg bg-slate-50 border border-slate-100 flex items-center gap-2">
              ...
              <span
                className={`text-sm font-bold ${sessionData.integrity_score < 70 ? "text-rose-600" : "text-emerald-600"}`}
              >
            ...
            {!isMobileConnected ? (
                ...
                <div className="w-12 h-12 rounded-xl bg-white flex items-center justify-center shadow-sm mb-4">
                  ...
                  >
                    <path
                      ...
                      strokeWidth="2"
                      d="M12 4v1m0 11v1m4-12h-1m-5 0H8m9 8h-1m-5 0H8m4-4a4 4 0 100 8 4 4 0 000-8z"
                    />
                  </svg>
                </div>
              ...
              <div className="py-6">
                ...
                <p className="text-sm text-emerald-600 font-medium mt-1">
                  Secondary view is live and active.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
        ...
        <div className="flex flex-col gap-1 text-right">
          ...
          <div className="flex items-center gap-1.5 justify-end">
            <div
              className={`w-1.5 h-1.5 rounded-full ${isMobileConnected ? "bg-emerald-500" : "bg-slate-300"}`}
            ></div>
            <span className="text-[10px] font-bold text-slate-700">
              {isMobileConnected ? "ONLINE" : "OFFLINE"}
            </span>
          </div>
        </div>
      </div>
