...
async def get_recruiter_stats(recruiter_id: Optional[str] = None):
    ...
    avg_score = round(avg_score_res[0]["avgScore"] * 10, 1) if avg_score_res else 0 # Scale 0-10 to 0-100%
    
    integrity_pass_query = result_query.copy()
    integrity_pass_query["integrity_score"] = {"$gt": 80}
...
async def get_candidate_stats(user_id: str):
    user_results = list(results_col.find({"candidate_id": user_id}).sort("_id", -1))
    if not user_results:
        return {
            "status": "Not Started",
            "percentile": "—",
    ...
    all_results = list(results_col.find({}, {"average_score": 1}))
    all_scores = [r.get("average_score", 0) for r in all_results]
    all_scores.sort()
    
    current_score = latest.get("average_score", 0)
    ...
    for res in user_results:
        ...
        history.append({
            ...
            "job_title": res.get("job_title", "Unknown Job"),
            "date": date_str,
            "score": res.get("average_score", 0),
            "time": f"{round(res.get('time_taken_seconds', 0) / 60, 1)}m",
    ...
    return {
        ...
        "timeTaken": f"{round(latest.get('time_taken_seconds', 0) / 60, 1)}m",
